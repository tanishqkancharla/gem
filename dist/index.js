(() => {
  ...
  function findOffsetInNode(node4, coords2) {
    var closest, dxClosest = 2e8, coordsClosest, offset2 = 0;
    var rowBot = coords2.top, rowTop = coords2.top;
    for (var child3 = node4.firstChild, childIndex = 0; child3; child3 = child3.nextSibling, childIndex++) {
      ...
      for (var i = 0; i < rects.length; i++) {
        ...
        if (rect.top <= rowBot && rect.bottom >= rowTop) {
          ...
          var dx = rect.left > coords2.left ? rect.left - coords2.left : rect.right < coords2.left ? coords2.left - rect.right : 0;
          if (dx < dxClosest) {
            closest = child3;
            dxClosest = dx;
            coordsClosest = dx && closest.nodeType == 3 ? { left: rect.right < coords2.left ? rect.right : rect.left, top: coords2.top } : coords2;
            if (child3.nodeType == 1 && dx) {
              offset2 = childIndex + (coords2.left >= (rect.left + rect.right) / 2 ? 1 : 0);
            }
            continue;
          }
        }
        if (!closest && (coords2.left >= rect.right && coords2.top >= rect.top || coords2.left >= rect.left && coords2.top >= rect.bottom)) {
  ...
  function findOffsetInText(node4, coords2) {
    ...
    for (var i = 0; i < len; i++) {
      ...
      if (inRect(coords2, rect)) {
        return { node: node4, offset: i + (coords2.left >= (rect.left + rect.right) / 2 ? 1 : 0) };
      }
    }
    return { node: node4, offset: 0 };
  }
  function inRect(coords2, rect) {
    return coords2.left >= rect.left - 1 && coords2.left <= rect.right + 1 && coords2.top >= rect.top - 1 && coords2.top <= rect.bottom + 1;
  }
  function targetKludge(dom, coords2) {
    var parent = dom.parentNode;
    if (parent && /^li$/i.test(parent.nodeName) && coords2.left < dom.getBoundingClientRect().left) {
      return parent;
    }
    return dom;
  }
  function posFromElement(view2, elt, coords2) {
    var ref = findOffsetInNode(elt, coords2);
    var node4 = ref.node;
    var offset2 = ref.offset;
    var bias = -1;
    if (node4.nodeType == 1 && !node4.firstChild) {
      var rect = node4.getBoundingClientRect();
      bias = rect.left != rect.right && coords2.left > (rect.left + rect.right) / 2 ? 1 : -1;
    }
    return view2.docView.posFromDOM(node4, offset2, bias);
  }
  function posFromCaret(view2, node4, offset2, coords2) {
    var outside = -1;
    for (var cur = node4; ; ) {
      if (cur == view2.dom) {
        break;
      ...
      if (desc.node.isBlock && desc.parent) {
        ...
        if (rect.left > coords2.left || rect.top > coords2.top) {
          outside = desc.posBefore;
        } else if (rect.right < coords2.left || rect.bottom < coords2.top) {
  ...
  function elementFromPoint(element, coords2, box) {
    var len = element.childNodes.length;
    if (len && box.top < box.bottom) {
      for (var startI = Math.max(0, Math.min(len - 1, Math.floor(len * (coords2.top - box.top) / (box.bottom - box.top)) - 2)), i = startI; ; ) {
        ...
        if (child3.nodeType == 1) {
          ...
          for (var j = 0; j < rects.length; j++) {
            var rect = rects[j];
            if (inRect(coords2, rect)) {
              return elementFromPoint(child3, coords2, rect);
            }
          }
        }
        if ((i = (i + 1) % len) == startI) {
  ...
  function posAtCoords(view2, coords2) {
    var assign, assign$1;
    var doc2 = view2.dom.ownerDocument, node4, offset2;
    if (doc2.caretPositionFromPoint) {
      try {
        var pos$1 = doc2.caretPositionFromPoint(coords2.left, coords2.top);
        if (pos$1) {
    ...
    if (!node4 && doc2.caretRangeFromPoint) {
      var range = doc2.caretRangeFromPoint(coords2.left, coords2.top);
      if (range) {
        assign$1 = range, node4 = assign$1.startContainer, offset2 = assign$1.startOffset;
      }
    }
    var elt = (view2.root.elementFromPoint ? view2.root : doc2).elementFromPoint(coords2.left, coords2.top + 1), pos;
    if (!elt || !view2.dom.contains(elt.nodeType != 1 ? elt.parentNode : elt)) {
      var box = view2.dom.getBoundingClientRect();
      if (!inRect(coords2, box)) {
        return null;
      }
      elt = elementFromPoint(view2.dom, coords2, box);
      if (!elt) {
        return null;
      }
    }
    ...
    if (node4) {
      if (result.gecko && node4.nodeType == 1) {
        ...
        if (offset2 < node4.childNodes.length) {
          ...
          if (next.nodeName == "IMG" && (box$1 = next.getBoundingClientRect()).right <= coords2.left && box$1.bottom > coords2.top) {
            offset2++;
          }
        }
      }
      if (node4 == view2.dom && offset2 == node4.childNodes.length - 1 && node4.lastChild.nodeType == 1 && coords2.top > node4.lastChild.getBoundingClientRect().bottom) {
        pos = view2.state.doc.content.size;
      } else if (offset2 == 0 || node4.nodeType != 1 || node4.childNodes[offset2 - 1].nodeName != "BR") {
        pos = posFromCaret(view2, node4, offset2, coords2);
      }
    }
    if (pos == null) {
      pos = posFromElement(view2, elt, coords2);
    }
  ...
  function endOfTextblockVertical(view2, state2, dir) {
    ...
    return withFlushedState(view2, state2, function() {
      ...
      var coords2 = coordsAtPos(view2, $pos.pos, 1);
      for (var child3 = dom.firstChild; child3; child3 = child3.nextSibling) {
        ...
        for (var i = 0; i < boxes.length; i++) {
          var box = boxes[i];
          if (box.bottom > box.top + 1 && (dir == "up" ? coords2.top - box.top > (box.bottom - coords2.top) * 2 : box.bottom - coords2.bottom > (coords2.bottom - box.top) * 2)) {
            return false;
          }
        }
      }
  ...
  EditorView.prototype.posAtCoords = function posAtCoords$1(coords2) {
    return posAtCoords(this, coords2);
  };
